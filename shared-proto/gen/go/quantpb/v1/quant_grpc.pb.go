...
func (c *quantClient) StreamBacktestProgress(ctx context.Context, in *GetBacktestStatusRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[BacktestProgress], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Quant_ServiceDesc.Streams[0], Quant_StreamBacktestProgress_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[GetBacktestStatusRequest, BacktestProgress]{ClientStream: stream}
...
type Quant_StreamBacktestProgressClient = grpc.ServerStreamingClient[BacktestProgress]
...
func (UnimplementedQuantServer) StreamBacktestProgress(*GetBacktestStatusRequest, grpc.ServerStreamingServer[BacktestProgress]) error {
...
func _Quant_StreamBacktestProgress_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(GetBacktestStatusRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(QuantServer).StreamBacktestProgress(m, &grpc.GenericServerStream[GetBacktestStatusRequest, BacktestProgress]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Quant_StreamBacktestProgressServer = grpc.ServerStreamingServer[BacktestProgress]
